  ...
  return (
    <section className="card">
      ...
      <div className="grid3 timers">
        <div className="timer">
          <div>
            <strong>K Car 3일 환불</strong>
          </div>
          <div className="due">{d3.toLocaleDateString()}</div>
          <div>{apiResponse?.deadlines?.d3?.label ?? leftStr(today, d3)}</div>
        </div>
        <div className="timer">
          <div>
            <strong>엔카 7일 환불</strong>
          </div>
          <div className="due">{d7.toLocaleDateString()}</div>
          <div>{apiResponse?.deadlines?.d7?.label ?? leftStr(today, d7)}</div>
        </div>
        <div className="timer">
          <div>
            <strong>성능보증 30일/2,000km</strong>
          </div>
      ...
      <p
        className="hint"
        style={{ fontSize: "13px", color: "#737373", marginTop: "12px" }}
      >
        인도일 기준 D-3, D-7, D-30/2,000km 등 주요 마감까지 남은 기간을
        표시합니다.
      </p>
    </section>
  );
